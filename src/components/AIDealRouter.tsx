    ...
    <div className="space-y-6">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div>
            ...
            <p className="text-xs sm:text-sm text-slate-600 dark:text-slate-400 mt-1 max-w-2xl">
              Paste a WhatsApp/Slack thread, Upwork offer, or client brief. The model extracts terms, calculates FX leakage, flags scope traps, and routes you to the exact gateway that delivers the highest PKR with SBP 0.25% tax compliance.
            </p>
          </div>
      ...
      <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-5 sm:p-6 shadow-sm transition-colors space-y-4">
        ...
        <textarea
          ...
          className="w-full text-xs sm:text-sm font-mono p-3.5 bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-xl focus:ring-2 focus:ring-indigo-500 focus:outline-none text-slate-900 dark:text-slate-100 placeholder-slate-400 transition-colors"
        />

        {/* Drag & Drop Screenshot / Image Area */}
        <div
          onDragOver={(e) => e.preventDefault()}
          onDrop={handleDrop}
          className="border-2 border-dashed border-slate-200 dark:border-slate-800 hover:border-indigo-400 dark:hover:border-indigo-600 rounded-xl p-3 sm:p-4 text-center transition-colors bg-slate-50/50 dark:bg-slate-950/50"
        >
          {imagePreview ? (
            <div className="flex items-center justify-between gap-3 max-w-md mx-auto p-2 bg-white dark:bg-slate-900 rounded-lg border border-slate-200 dark:border-slate-800">
              <img
                src={imagePreview}
              ...
              <button
                type="button"
                onClick={removeImage}
                className="px-2 py-1 text-xs text-rose-600 hover:bg-rose-50 dark:hover:bg-rose-950/50 rounded font-bold"
              >
                Remove
              </button>
            </div>
          ) : (
            <label className="cursor-pointer flex flex-col sm:flex-row items-center justify-center gap-2 text-xs text-slate-500 dark:text-slate-400">
              <Upload className="w-4 h-4 text-indigo-500" />
              <span>Or drag & drop client chat screenshot / invoice receipt</span>
              <span className="text-[11px] text-indigo-600 dark:text-indigo-400 font-bold underline">
                Browse file
              </span>
              <input
                type="file"
